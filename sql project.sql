...
3. Including the department name, 
return the departments and the average salary of employees of each department. 
List the top 3 department with the highest number of employees 
...
select count(employee_id)from employees where department_id=6
-------------------------------
select D.department_name,E.department_id,count(employee_id) from employees E  inner join departments D  using (department_id)
...
.Also select only departments that have more than five employees*/
...
select Round(sum(salary),0) Salary from employees
